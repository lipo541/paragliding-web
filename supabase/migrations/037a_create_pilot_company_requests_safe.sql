-- =====================================================
-- Migration 037: Create Pilot Company Requests Table (SAFE VERSION)
-- =====================================================
-- This version checks for existing objects before creating
-- =====================================================

-- 1. Create request status ENUM (only if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'pilot_company_request_status') THEN
        CREATE TYPE pilot_company_request_status AS ENUM ('pending', 'approved', 'rejected');
    END IF;
END $$;

-- 2. Create pilot_company_requests table (only if not exists)
CREATE TABLE IF NOT EXISTS pilot_company_requests (
...
CREATE INDEX IF NOT EXISTS idx_pilot_company_requests_pilot_id ON pilot_company_requests(pilot_id);
CREATE INDEX IF NOT EXISTS idx_pilot_company_requests_company_id ON pilot_company_requests(company_id);
CREATE INDEX IF NOT EXISTS idx_pilot_company_requests_status ON pilot_company_requests(status);
CREATE INDEX IF NOT EXISTS idx_pilot_company_requests_created_at ON pilot_company_requests(created_at DESC);

-- 4. Enable Row Level Security
ALTER TABLE pilot_company_requests ENABLE ROW LEVEL SECURITY;

-- 5. Drop existing policies if they exist, then recreate
DROP POLICY IF EXISTS "Pilots can view own requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Pilots can create requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Pilots can delete pending requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Companies can view their requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Companies can respond to requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Super admin can view all pilot company requests" ON pilot_company_requests;
DROP POLICY IF EXISTS "Super admin can update any pilot company request" ON pilot_company_requests;
DROP POLICY IF EXISTS "Super admin can delete any pilot company request" ON pilot_company_requests;
...
DROP TRIGGER IF EXISTS pilot_company_requests_updated_at_trigger ON pilot_company_requests;
...
-- 7. Create function to approve pilot request
