...
export default function AddLocation({ countryId, countryName, onClose, onSuccess }: AddLocationProps) {
  ...
  const mapSrc = useMemo(() => {
    return extractIframeSrc(locationForm.mapIframeUrl);
  }, [locationForm.mapIframeUrl]);
  ...
  const handleSaveLocation = async () => {
    try {
      ...
      if (ogImage) {
        ...
        const fileName = `locations/${locationId}/og-image.${fileExt}`;

        console.log('=== Storage Upload Debug ===');
        console.log('Bucket: countries-og-images');
        console.log('File name:', fileName);
        console.log('File size:', ogImage.size);
        console.log('File type:', ogImage.type);
        ...
        if (uploadData) {
          ...
          console.log('Image uploaded successfully:', ogImageUrl);
        }
      }
      ...
      if (error) {
        ...
        alert(`მონაცემთა ბაზაში ჩაწერის შეცდომა: ${error.message}`);
        setIsSaving(false);
        return;
      }

      alert("ლოკაცია წარმატებით დაემატა!");
      onSuccess();
      onClose();
    } catch (err: any) {
    ...
    <div className="mb-6 bg-background border-2 border-green-600/30 rounded-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-semibold text-foreground">
          ახალი ლოკაციის დამატება - <span className="text-green-600">{countryName}</span>
        </h3>
        <button
          onClick={onClose}
      ...
      <div className="mb-6 p-4 bg-foreground/5 rounded-lg border border-foreground/10">
        ...
        <div className="space-y-3">
          <textarea
            value={locationForm.mapIframeUrl}
            onChange={(e) => setLocationForm(prev => ({ ...prev, mapIframeUrl: e.target.value }))}
            ...
            rows={4}
            className="w-full px-4 py-2 bg-background border border-foreground/20 rounded-md text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-green-500 font-mono"
          />
