...
interface LocationContextType {
  ...
  // Delete images from storage and state
...
export function LocationProvider({ children }: { children: ReactNode }) {
  ...
  const deleteHeroImage = async () => {
    // თუ pending სურათია (ჯერ არ ატვირთული), უბრალოდ გავასუფთავოთ
    ...
    // თუ შენახული სურათია bucket-ში
    if (sharedImages.hero_image?.url) {
      const url = sharedImages.hero_image.url;
      // URL-დან ვიღებთ ფაილის path-ს
      const urlParts = url.split('/locationsIMG-gallery/');
      if (urlParts.length > 1) {
        const filePath = decodeURIComponent(urlParts[1]);
        const { error } = await supabase.storage.from('locationsIMG-gallery').remove([filePath]);
        if (error) {
