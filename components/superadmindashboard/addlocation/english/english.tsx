...
export default function EnglishForm() {
  ...
  const enContent = languageContent.en;
  ...
  const flightTypes = enContent.flight_types || [];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
        <p className="text-sm text-blue-800 dark:text-blue-200 flex items-center gap-2">
          <span>🇬🇧</span>
          <span>ინგლისური ფორმა - შეავსეთ ინგლისურად</span>
        </p>
      </div>
      ...
      {imagePreviews.heroPreview && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
          <p className="text-sm text-green-800 dark:text-green-200 flex items-center gap-2">
            ...
            სურათები ატვირთულია ქართულ ფორმაში. ამ ფორმაში მხოლოდ ALT ტექსტები დაწერეთ ინგლისურად.
          </p>
        </div>
      )}
      ...
      {imagePreviews.heroPreview && (
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">
            <span className="text-orange-600 dark:text-orange-400">*</span> მთავარი სურათის ALT ტექსტი (ინგლისურად)
          </label>
          <div className="space-y-3">
            ...
            <input
              type="text"
              value={pendingImages.heroImage?.alt_en || sharedImages.hero_image?.alt_en || ""}
              onChange={(e) => {
                // Update pendingImages if exists (before save)
                if (pendingImages.heroImage) {
                  setPendingImages({
                    ...pendingImages,
                    heroImage: { ...pendingImages.heroImage, alt_en: e.target.value }
                  });
                }
                ...
                if (sharedImages.hero_image) {
                  setSharedImages({
                    ...sharedImages,
                    hero_image: { ...sharedImages.hero_image, alt_en: e.target.value }
                  });
                }
              }}
              ...
              placeholder="e.g: Paragliding over Gudauri mountains"
            />
            <p className="text-xs text-foreground/60">
              ⚠️ ინგლისურად დაწერეთ ALT აღწერა ამ სურათისთვის
            </p>
          </div>
        </div>
      )}
      ...
      {imagePreviews.galleryPreviews.length > 0 && (
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">
            <span className="text-orange-600 dark:text-orange-400">*</span> გალერეის სურათების ALT ტექსტები (ინგლისურად)
          </label>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {imagePreviews.galleryPreviews.map((img, index) => (
              <div key={index} className="space-y-2">
                ...
                <input
                  type="text"
                  value={pendingImages.galleryImages[index]?.alt_en || sharedImages.gallery[index]?.alt_en || ""}
                  onChange={(e) => {
                    // Update pendingImages if exists (before save)
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${sharedVideos.videoUrls.length > 0 ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> YouTube Videos
        </label>
        ...
        <div className="space-y-3">
          ...
          <div className="border-2 border-dashed border-foreground/20 rounded-lg p-4 bg-foreground/5">
            ...
            <p className="text-xs text-foreground/60 mt-2 flex items-center gap-1">
              ...
              Paste the full YouTube video URL and press Enter or click Add button
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.h1_tag?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> H1 Tag
        </label>
        <input
          type="text"
          value={enContent.h1_tag || ""}
          onChange={(e) => updateLanguageContent('en', { h1_tag: e.target.value })}
          className="w-full px-3 py-2 text-sm border border-foreground/20 rounded-lg focus:ring-2 focus:ring-foreground/20 focus:border-foreground/40 outline-none transition-all bg-background text-foreground"
          placeholder="e.g: Paragliding in Gudauri"
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.p_tag?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> P Tag (აღწერა)
        </label>
        <textarea
          value={enContent.p_tag || ""}
          onChange={(e) => {
            updateLanguageContent('en', { p_tag: e.target.value });
          ...
          placeholder="e.g: Discover a unique flying experience in Georgia's most beautiful mountain region..."
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.h2_history?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> H2 Tag (ისტორია)
        </label>
        <input
          type="text"
          value={enContent.h2_history || ""}
          onChange={(e) => updateLanguageContent('en', { h2_history: e.target.value })}
          className="w-full px-3 py-2 text-sm border border-foreground/20 rounded-lg focus:ring-2 focus:ring-foreground/20 focus:border-foreground/40 outline-none transition-all bg-background text-foreground"
          placeholder="e.g: History of Gudauri"
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.history_text?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> ისტორიის ტექსტი
        </label>
        <RichTextEditor
          value={enContent.history_text || ""}
          onChange={(value) => updateLanguageContent('en', { history_text: value })}
          placeholder="e.g: Gudauri is one of Georgia's oldest and most popular ski resorts..."
          minHeight="200px"
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.gallery_description?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> გალერეის აღწერა
        </label>
        <textarea
          value={enContent.gallery_description || ""}
          onChange={(e) => {
            updateLanguageContent('en', { gallery_description: e.target.value });
            e.target.style.height = 'auto';
            e.target.style.height = e.target.scrollHeight + 'px';
          }}
          className="w-full px-3 py-2 text-sm border border-foreground/20 rounded-lg focus:ring-2 focus:ring-foreground/20 focus:border-foreground/40 outline-none transition-all bg-background text-foreground resize-none overflow-hidden"
          style={{ minHeight: '60px' }}
          placeholder="e.g: Photos from different angles of flight and panoramic views..."
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          <span className={`${enContent.h3_flight_types?.trim() ? 'text-green-600 dark:text-green-400' : 'text-foreground/40'}`}>○</span> H3 Tag (ფრენის ტიპები)
        </label>
        <input
          type="text"
          value={enContent.h3_flight_types || ""}
          onChange={(e) => updateLanguageContent('en', { h3_flight_types: e.target.value })}
          className="w-full px-3 py-2 text-sm border border-foreground/20 rounded-lg focus:ring-2 focus:ring-foreground/20 focus:border-foreground/40 outline-none transition-all bg-background text-foreground"
          placeholder="e.g: Flight Types in Gudauri"
        />
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
        <p className="text-sm text-blue-800 dark:text-blue-200 flex items-start gap-2">
          ...
          <span><strong>Note:</strong> New flight types must be created in the Georgian form first. Here you can only translate existing flight types.</span>
        </p>
      </div>
        ...
        return untranslated.length > 0 && (
          <div className="border-2 border-dashed border-orange-300 dark:border-orange-700 rounded-lg p-4 bg-orange-50/50 dark:bg-orange-900/10">
            <h4 className="text-sm font-semibold text-orange-800 dark:text-orange-300 mb-3 flex items-center gap-2">
              ...
              Flight types that need translation
            </h4>
              ...
              <div key={index} className="border border-foreground/20 rounded-lg p-4 bg-foreground/5">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={() => {
                        // Only delete this language's translation
              ...
              {flightTypeFeatures.map((feature, index) => (
                <div key={index} className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="e.g: Flight duration: 15-20 minutes"
                  />
                  <button
                    onClick={() => {
