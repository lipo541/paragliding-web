...
interface CompanyVideoUrlsProps {
  companyId: string;
  videoUrls: string[];
...
const isValidVideoUrl = (url: string): boolean => {
  const youtubeRegex = /^(https?:\/\/)?(www\.)?(youtube\.com\/(watch\?v=|embed\/|v\/|playlist\?list=)|youtu\.be\/)[\w-]+/;
...
const getVideoThumbnail = (url: string): string | null => {
  ...
  // YouTube playlist - use first video thumbnail if available
  const playlistMatch = url.match(/youtube\.com\/playlist\?list=([a-zA-Z0-9_-]+)/);
  if (playlistMatch) {
    return null; // Playlist doesn't have a single thumbnail
  }
...
export default function CompanyVideoUrls({
  companyId,
  videoUrls = [],
  onUpdate,
  maxVideos = 10,
}: CompanyVideoUrlsProps) {
  ...
  const addVideo = useCallback(async () => {
    ...
    try {
      ...
      setNewUrl('');
      toast.success(isPlaylist(newUrl.trim()) ? 'Playlist დამატებულია' : 'ვიდეო დამატებულია');
    } catch (err) {
  ...
  }, [newUrl, videoUrls, maxVideos, companyId, onUpdate, supabase]);
  ...
  }, [videoUrls, companyId, onUpdate, supabase]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h4 className="text-sm font-medium text-[#1a1a1a] dark:text-white">
            ვიდეოები ({videoUrls.length}/{maxVideos})
          </h4>
          <p className="text-xs text-[#1a1a1a]/50 dark:text-white/50">
            დაამატეთ YouTube ვიდეოები ან Playlist
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-[#1a1a1a]/40 dark:text-white/40">
          მაგ: https://www.youtube.com/watch?v=... ან https://www.youtube.com/playlist?list=...
        </p>
      </div>
        ...
        <div className="space-y-3">
          {videoUrls.map((url, index) => {
            ...
            return (
              ...
              >
                {/* Thumbnail */}
                <div className="w-24 h-14 rounded overflow-hidden bg-black/20 flex-shrink-0 relative">
                  {thumbnail ? (
                    <img
                      src={thumbnail}
                      alt={`Video ${index + 1}`}
                  ...
                  {isPlaylistUrl && (
                    <div className="absolute bottom-0 right-0 bg-black/70 text-white text-[9px] px-1 py-0.5">
                      Playlist
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isPlaylistUrl && (
                    <span className="text-[10px] text-[#4697D2]">YouTube Playlist</span>
                  )}
                </div>
