...
interface CompanyGalleryUploadProps {
  companyId: string;
  userId: string;
  images: GalleryImage[];
...
export default function CompanyGalleryUpload({
  companyId,
  userId,
  images = [],
  onUpdate,
  maxImages = 20,
}: CompanyGalleryUploadProps) {
  ...
  }, [images, maxImages, onUpdate, companyId, supabase, userId]);
  ...
  }, [images, onUpdate, companyId, supabase]);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-medium text-[#1a1a1a] dark:text-white">
            ფოტო გალერეა ({images.length}/{maxImages})
          </h4>
          <p className="text-xs text-[#1a1a1a]/50 dark:text-white/50">
            ატვირთეთ კომპანიის ფოტოები
          </p>
        </div>
        <button
