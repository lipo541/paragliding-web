...
| თარიღი | პრობლემა | გადაწყვეტა | ფაილები |
|--------|----------|------------|---------|
| **2025-11-28** | Sitemap-ში `lastModified` ყოველთვის დღევანდელი თარიღი იყო (`new Date()`), Google არ ენდობოდა | სტატიკური გვერდებისთვის ფიქსირებული თარიღი, დინამიურისთვის `updated_at` ბაზიდან | `app/sitemap.ts` |
| **2025-11-28** | Client-side Supabase (`createClient`) გამოიყენებოდა server-side კოდში | შეიცვალა `createServerClient`-ით | `app/sitemap.ts` |
| **2025-11-28** | `dynamic = 'force-static'` ბლოკავდა ISR revalidation-ს | წაიშალა, დარჩა მხოლოდ `revalidate = 86400` | `app/sitemap.ts` |
| **2025-11-28** | Sitemap cache header მოკლე იყო (1 საათი) | გაიზარდა 24 საათამდე | `next.config.ts` |
